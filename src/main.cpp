...
bool CTxOut::IsDust() const
{
    ...
    // so dust is a txout less than 54 uPXL
...
unsigned int static KimotoGravityWell(const CBlockIndex* pindexLast, const CBlockHeader *pblock, uint64 TargetBlocksSpacingSeconds, uint64 PastBlocksMin, uint64 PastBlocksMax) {
        /* current difficulty formula, KGW by Dr. Kimoto Chan */
...
{
static const int64	BlocksTargetSpacing	= 7 * 60; // 420
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        int DiffMode = 1;
        if (fTestNet) {
                if (pindexLast->nHeight+1 >= 5500) { DiffMode = 2; }
        }
        else {
                if (pindexLast->nHeight+1 >= 17447) { DiffMode = 2; }
        }
...
void SetGenesisHash() {
        // Genesis Block:
        // CBlock(hash=000000000019d6, ver=1, hashPrevBlock=00000000000000, hashMerkleRoot=4a5e1e, nTime=1389693873, nBits=1d00ffff, nNonce=2083236893, vtx=1)
        ...
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 0xd578c918 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
...
unsigned char pchMessageStart[4] = { 0xd5, 0x78, 0xc9, 0x18 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PxlcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PxlcoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("PxlcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("pxlcoin-miner");
    ...
    try { loop {
        ...
        printf("Running PxlcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PxlcoinMiner terminated\n");
        throw;
    }
}
